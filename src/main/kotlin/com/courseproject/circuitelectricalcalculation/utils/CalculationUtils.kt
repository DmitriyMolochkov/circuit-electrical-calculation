...
class CalculationUtils {
    ...
    public fun getCalculationResults(calculationCreateModel: CalculationCreateModel): CalculationResults {
        ...
        val resultForCTS = CalculationResultForCTS(
            vBranchesCurrents.getElementCopy(3, 0),
            vBranchesCurrents.getElementCopy(5, 0),
            vBranchesCurrents.getElementCopy(4, 0)
        )
        ...
        val resultsForTowers = calculationCreateModel.calculationPoints.map { p ->
            ...
            CalculationResultForTower(
                p,
                vBranchesVoltage.getElementCopy((index + 1) * 7, 0 ),
                vBranchesVoltage.getElementCopy((index + 1) * 7 + 2, 0),
                vBranchesVoltage.getElementCopy((index + 1) * 7 + 1, 0),
            )
        }

        return CalculationResults(vBranchesVoltage, vBranchesCurrents, resultForCTS, resultsForTowers);
    }
